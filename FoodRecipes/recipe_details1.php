...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_recipe'])) {
    ...
    if ($conn->query($sql)) {
        header("Location: entrees.php"); // Redirect to entrees page
        exit();
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_recipe'])) {
    ...
    if ($conn->query($sql)) {
        header("Location: recipe_details1.php?recipe_id=$recipe_id"); // Refresh the details page
        exit();
    } else {
    ...
    if (isset($_FILES['recipe_image']) && $_FILES['recipe_image']['error'] == 0) {
        ...
        if (in_array($imageFileType, ['jpg', 'jpeg', 'png', 'gif'])) {
            if (move_uploaded_file($_FILES['recipe_image']['tmp_name'], $target_file)) {
                ...
                if ($conn->query($sql)) {
                    header("Location: recipe_details1.php?recipe_id=$recipe_id");
                    exit();
                } else {
