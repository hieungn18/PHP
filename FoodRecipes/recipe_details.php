...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_recipe'])) {
    ...
    if ($conn->query($sql)) {
        header("Location: appetizers.php"); // Redirect to appetizers page
        exit();
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_recipe'])) {
    ...
    if ($conn->query($sql)) {
        header("Location: recipe_details.php?recipe_id=$recipe_id"); // Refresh the details page
        exit();
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['upload_image'])) {
    ...
    if (!is_dir($target_dir)) {
        mkdir($target_dir, 755, true); // Ensure the directory exists
    }
...
<body>
    <header>
        <div id="home-button">
            <a href="appetizers.php">Back to Appetizers</a>
        </div>
