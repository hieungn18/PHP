...
// Fetch all entree recipes
$sql = "SELECT * FROM recipes WHERE category = 'Entree' ORDER BY created_at DESC";
...
$sql = "SELECT * FROM recipes WHERE category = 'Entree' ORDER BY created_at DESC LIMIT $limit OFFSET $offset";
$result = $conn->query($sql);

// Count total recipes for pagination
$total_sql = "SELECT COUNT(*) AS total FROM recipes WHERE category = 'Entree'";
...
<body>
    ...
    <div id="container">
        <h1>Satisfying Entree Recipes</h1>
		
		<div id="add-recipe-button">
            <a href="add_recipe1.php">Add New Recipe</a>
        </div><br><br>
		...
		<?php while ($row = $result->fetch_assoc()) : ?>
			<div class="recipe">
				...
				<h2><a href="recipe_details1.php?recipe_id=<?= $row['recipe_id'] ?>"><?= htmlspecialchars($row['title']) ?></a></h2>
